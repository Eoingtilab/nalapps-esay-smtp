...
- [ ] Product-native serial entry, activate/check/deactivate flows are present and capability+nonce protected.
- [ ] License keys and the SMTP password are excluded from diagnostics and data export.
- [ ] WordPress Plugins update and internal `지금 업데이트` both use an installable EDD package URL.
...
- [ ] Settings export/import works, always strips `password_enc`, and import creates a pre-import snapshot.
...
- [ ] Production ZIP contains `vendor/easy-digital-downloads/edd-sl-sdk/edd-sl-sdk.php` and all maintenance runtime files, with dev-only tooling (WPCS/PHPCS) excluded.
